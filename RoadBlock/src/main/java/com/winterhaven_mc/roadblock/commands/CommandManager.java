...
public final class CommandManager implements CommandExecutor, TabCompleter {
	...
	public final boolean onCommand(final CommandSender sender, final Command command,
		...
		if (args.length < minArgs) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_ARGS_COUNT_UNDER).send();
		...
		if (args.length > maxArgs) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_ARGS_COUNT_OVER).send();
			plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
			return false;
		}
		...
		new Message<MessageId, Macro>(sender, COMMAND_FAIL_INVALID_COMMAND).send();
		plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
		return false;
	}
	...
	private boolean statusCommand(final CommandSender sender, String[] args) {
		...
		if (!sender.hasPermission("roadblock.status")) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_STATUS_PERMISSION).send();
			plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
		}
	...
	private boolean reloadCommand(final CommandSender sender, String[] args) {
		...
		if (!sender.hasPermission("roadblock.reload")) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_RELOAD_PERMISSION).send();
			plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
			return true;
		}
	...
	private boolean showCommand(final CommandSender sender, String[] args) {
		...
		if (!(sender instanceof Player)) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_CONSOLE).send();
			return true;
		}
		...
		if (!player.hasPermission("roadblock.show")) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_SHOW_PERMISSION).send();
		...
		if (args.length == 2) {
			try {
				...
				new Message<MessageId, Macro>(sender, COMMAND_FAIL_SET_INVALID_INTEGER).send();
				plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
	...
	private boolean toolCommand(final CommandSender sender, String[] args) {
		...
		if (!player.hasPermission("roadblock.tool")) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_TOOL_PERMISSION).send();
			plugin.soundConfig.playSound(player, SoundId.COMMAND_FAIL);
			return true;
		}
		...
		if (!noFit.isEmpty()) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_TOOL_INVENTORY_FULL).send();
			plugin.soundConfig.playSound(player, SoundId.COMMAND_FAIL);
			return true;
		}
	...
	private boolean helpCommand(final CommandSender sender, final String[] args) {
		...
		if (!sender.hasPermission("roadblock.help")) {
			new Message<MessageId, Macro>(sender, COMMAND_FAIL_HELP_PERMISSION).send();
			plugin.soundConfig.playSound(sender, SoundId.COMMAND_FAIL);
			return true;
		}
